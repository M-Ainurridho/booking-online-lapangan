<template>
   <div class="offcanvas offcanvas-end px-4" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
      ...
      <div class="offcanvas-body" style="display: grid">
         <div v-if="store.cart.schedules.length < 1">
            <p class="text-opacity-50 text-center fs-6">Belum ada jadwal di keranjang</p>
         </div>

         <div v-else>
            <div class="row" v-for="cart in store.cart.schedules" :key="cart.fieldName">
               <h6 class="fw-semibold" style="margin-left: -10px !important">{{ cart.fieldName }}</h6>
               <div class="col-12 cart-schedule bg-body-tertiary rounded-2 py-2 mb-3 overflow-hidden">
                  <div class="row">
                     <div class="col-10">
                        <div class="fw-medium">{{ cart.date }} <i class="bx bxs-circle d-inline-block" style="font-size: 5px !important; transform: translateY(-3px)"></i> {{ cart.start }} - {{ cart.end }}</div>
                        <div class="">
                           {{ cart.price }}
                        </div>
                     </div>
                     <div class="col-2 my-auto"><i class="bx bx-trash-alt fs-4"></i></div>
                  </div>
               </div>
            </div>
         </div>

         <div v-show="store.cart.schedules.length > 0" style="align-self: self-end">
